...
static int is_zero_area(box_t box) {
  if (compare_double(box.w * box.h, 0) == 0) return 1;
  else return 0;
}
...
static box_t triangle_bounding_box(triangle_t t) {
  ...
  // find mininmal point
  double x_min = t.a.x;
  if (x_min > t.b.x) x_min = t.b.x;
  if (x_min > t.c.x) x_min = t.c.x;
  double y_min = t.a.y;
  if (y_min > t.b.y) y_min = t.b.y;
  if (y_min > t.c.y) y_min = t.c.y;
  point_t point;
  point.x = x_min;
  point.y = y_min;
  box.p = point;
  ...
  // find maximum point
  double x_max = t.a.x;
  if (x_max < t.b.x) x_max = t.b.x;
  if (x_max < t.c.x) x_max = t.c.x;
  double y_max = t.a.y;
  if (y_max < t.b.y) y_max = t.b.y;
  if (y_max < t.c.y) y_max = t.c.y;
  box.w = x_max - x_min;
  box.h = y_max - y_min;

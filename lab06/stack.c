...
stack_t *stack_new(size_t max_elements)
{
	...
	instance = malloc(3 * sizeof(int));
	if (instance == NULL) error_fatal_errno("stack_new: instance");
	instance->stack = malloc(max_elements * sizeof(int));
	if (instance->stack == NULL) error_fatal_errno("stack_new: stack");
	instance->top = instance->stack;
	instance->full = (instance->stack + max_elements);
...
int stack_is_empty(stack_t *instance)
{
	...
	if (instance->top != instance->stack) is_empty = 0;
...
int stack_is_full(stack_t *instance)
{
	...
	if (instance->top != instance->full) is_full = 0;
